 ...
 *
 * Kept here rather than in the component that renders them, because the shared image drawn
 * on a win uses the same paths. Two copies would drift, and the drift would only show up in
 * something a player had already posted somewhere.
 *
 * Drawn for this project. Do not paste in path data from a piece set, free or otherwise.
 * See CLAUDE.md rule 0.
 */

import type { PieceType } from "./chess/types.ts";
import { BISHOP, KING, KNIGHT, PAWN, QUEEN, ROOK } from "./chess/types.ts";

/** Every glyph is drawn in this box. */
export const PIECE_VIEWBOX = 32;
...
export const PIECE_MARKS: Record<PieceType, string> = {
